...
export function SyncPanel({ account, state, commit, c, t, error, sync, pull, back, exportJson }: Props) {
  ...
  return <View style={{ gap: 16 }}>
    ...
    <Text style={{ color: c.muted }}>{t("Cloud hiện lưu một bản thư viện, chưa tự gộp thay đổi từ hai máy. Nếu có xung đột, app giữ dữ liệu trên máy và báo lỗi. Xuất JSON trước, rồi chọn tải bản cloud nếu muốn thay thế. Bản cloud không phải lịch sử sao lưu nhiều phiên bản.", "Cloud stores one library snapshot and does not merge edits from two devices. A conflict keeps local data and reports an error. Export JSON first, then load the cloud copy if you want to replace it. Cloud is not a versioned backup history.")}</Text>
    {account ? <>
      ...
      {!state.books.length && <Button c={c} secondary disabled={busy || !!state.draft} label={t("Sao chép tủ sách local vào tài khoản", "Copy local library into account")} onPress={() => Alert.alert("ReadSession", t("Sao chép tủ sách local vào tài khoản và đồng bộ? Sách và ghi chú bắt đầu ở private. Bản local vẫn còn.", "Copy the local library into this account and sync it? Books and notes start private. The local library remains."), [{ text: t("Hủy", "Cancel"), style: "cancel" }, { text: t("Sao chép", "Copy"), onPress: () => commit(current => current.books.length ? current : { ...privateImport(guestLibrary()), cloudRevision: current.cloudRevision ?? 0 }) }])} />}
      <Button c={c} secondary disabled={busy || !!state.draft} label={t("Đăng xuất, dùng local", "Sign out and use local")} onPress={() => {
